    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                var callbackUrl = Url.Action("ResetPassword", "Account", new { userId = users.Id, code = code }, protocol: Request.Url.Scheme);
                await UserManager.SendEmailAsync(users.Id, "Reset Password", "Please reset your password by clicking <a href=\"" + callbackUrl + "\">here</a>");
        ...
        public async Task<ActionResult> ExternalLoginConfirmation(ExternalLoginConfirmationViewModel model, string returnUrl)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    result = await UserManager.AddLoginAsync(users.Id, info.Login);
                    if (result.Succeeded)
                    {
                        await SignInManager.SignInAsync(users, isPersistent: false, rememberBrowser: false);
                        return RedirectToLocal(returnUrl);
